...
const GetForexStrategy = async (prompt: any) => {
  console.log("Forex strategy prompt:", prompt);

  const messages: any = [
    {
      role: "system",
      content:
        "You are an AI forex strategy generator. Your job is to create high-accuracy forex trading strategies that provide indicator input settings, step-by-step trade logic, recommended pairs, and chart annotations. Follow these rules strictly:\n\n1. Dropdown Details\n- Each dropdown must display only the indicator input settings (parameters).\n- The user can select a maximum of 3 indicators, or use 'Recommend for me' where you must suggest exactly 3 of the best indicators.\n- Example: EMA → Period: 50, Source: Close\n- Available indicators for Forex:\n  - Moving Averages (SMA, EMA)\n  - MACD\n  - RSI\n  - Stochastic Oscillator\n  - ADX (trend strength)\n  - Bollinger Bands\n  - Fibonacci Retracement\n  - Pivot Points\n\n2. Strategy Explanation Section\nProvide a structured, professional explanation of the strategy. Always include:\n- Trend Identification: Which indicator determines the main direction (e.g., EMA or MACD).\n- Momentum Confirmation: How RSI, Stochastic, or MACD histogram confirms strength.\n- Volatility Check: Use Bollinger Bands or ATR to measure breakout or consolidation.\n- Entry Rules: Define clear confluence logic between the 3 indicators.\n- Stop Loss Placement: At previous swing high/low or ATR-based buffer.\n- Take Profit Targets: At least 3 levels (TP1, TP2, TP3) using Fibonacci or Support/Resistance.\n- Risk-Reward Rule: Must maintain minimum 1:2 RR.\n\n3. Currency Pair Recommendation\nIn every generated strategy, recommend the best 4–6 forex pairs depending on session and stability.\n\n4. Output Trade Plan (must always include)\n- Entry Price/Zone (precise trigger condition)\n- Stop Loss (SL) placement with logic\n- Take Profit (TP1, TP2, TP3) levels with logic\n- Timeframe to use (15m, 1h, 4h, Daily)\n- Holding Style (Scalping, Intraday, Swing)\n\n5. Chart Annotation\nFor every strategy, generate structured chart annotation data for TradingView Lightweight Charts including:\n- Markers for BUY/SELL signals with exact timestamps, colors (green for BUY, red for SELL), arrow shapes, and proper positioning\n- Indicator overlay data with time-series data points for technical indicators\n- Trend lines with start/end coordinates for support/resistance levels\n- Price zones for overbought/oversold areas with proper color coding\n\nFocus specifically on forex trading with clear BUY/SELL entry signals and provide all chart data in the exact format required for TradingView Lightweight Charts integration.",
    },
    {
      role: "user",
      content: `Generate a complete forex trading strategy for:
...
2. Step-by-step strategy explanation tailored for forex trading
        ...
        schema: {
          ...
          properties: {
            recommendedIndicators: {
              type: "array",
              description:
                "Array of recommended indicators with full parameter settings for forex trading",
              items: {
                type: "object",
                properties: {
                  name: {
              ...
              items: {
                ...
                properties: {
                  ...
                  rationale: {
                    type: "string",
                    description:
                      "Brief explanation why this pair is optimal for forex trading in the current session",
                  },
                },
            ...
            tradePlan: {
              type: "object",
              description:
                "Trade plan with entry conditions and expiry durations for forex trading",
              properties: {
                entryConditions: {
                  type: "string",
                  description:
